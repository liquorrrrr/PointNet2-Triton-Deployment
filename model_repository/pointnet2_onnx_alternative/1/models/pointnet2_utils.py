...
def farthest_point_sample(xyz, npoint):
    ...
    torch.cuda.nvtx.range_push(f"FPS_Operator_{mode}")
    
    if mode == "pytorch":
        out = farthest_point_sample_pytorch(xyz, npoint)
    elif mode == "triton_v1":
        out = triton_fps_v1(xyz, npoint)
    elif mode == "triton_v2":
        out = triton_fps_v2(xyz, npoint)
    else:
...
def query_ball_point(radius, nsample, xyz, new_xyz):
    ...
    torch.cuda.nvtx.range_push(f"BQ_Operator_{mode}")
    if mode == "pytorch":
        device = xyz.device
        B, N, C = xyz.shape
        _, S, _ = new_xyz.shape
        ...
        out = triton_bq_v1(xyz, new_xyz, radius, nsample)
    else:
        raise ValueError(f"Unknown mode: {mode}")
    torch.cuda.nvtx.range_pop()
    return out
